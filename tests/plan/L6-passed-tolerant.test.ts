 ...
 *
 * 前提断言的 `some(!passed)` 是**状态锁**，全验收后必然红：M6 验收（2026-08-23，
 * fe373da 标 M6 ✅）后 M1–M6 全带 ✅，该断言不可满足——拦的不是 bug 是合法终点。
 * 按本条自己的教训（锁规则不锁状态）：保留 `some(passed)`（防「全部不带 ✅ 也能绿」
 * 的真空绿，对应关系判据仍逐条验伪），删除 `some(!passed)`。
 *
...
describe("L6 已验收标记的容忍", () => {
  it("标题带 ✅ ⇔ passed=true（逐条对齐原文，不写死哪几个）", () => {
    ...
    try {
      ...
      // 前提断言：至少有一个已验收（防「全部不带 ✅ 也能绿」——数据不能自己定义
      // 通过条件，与 L7 那条 owner 被写松同一个坑）。
      // 不要求「存在未验收」：那是状态锁，全验收（M6 后）后不可满足（见文件头注释）。
