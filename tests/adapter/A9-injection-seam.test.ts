...
describe("A9 注入缝", () => {
  ...
  it("同进程 wire() 三次 → A 的 fake pi 上没收到过 B 的事件（注册隔离）", () => {
    ...
    try {
      ...
      piA.emit("agent_end", { messages: [{ role: "user", content: "wf: 收到 task_assignment（arch → dev，M1）：造文件", timestamp: 1 }] }, { cwd: p.root, mode: "tui" });
  ...
  it("A 的 fake pi 上没收到过 B 注册的工具（注册隔离，反向）", () => {
    ...
    try {
      ...
      piB.emit("agent_end", { messages: [{ role: "user", content: "wf: 收到 task_assignment（arch → dev，M1）：造文件", timestamp: 1 }] }, { cwd: p.root, mode: "tui" });
