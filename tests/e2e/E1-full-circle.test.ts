...
import { fakePi, makeProject, realConfig } from "../adapter/_fixture.ts";
...
describe("E1 完整一圈", () => {
  it("分发 → 产出 → FAIL → 修 → PASS → /pass → 回 arch", async () => {
    ...
    const root = p.root;
    try {
      const { cfg } = realConfig(root, { source: "src", test: `node -e "console.log('passed')"` });
      ...
      // ── 2. dev 没写产出就投 review_request → G-artifact 拦 ──────
      const bad = { type: "review_request", milestone: "M1", body: "做完了", refs: [] };
      const blocked = intercept.dev(bad);
      expect(blocked).toMatchObject({ block: true });
      expect((blocked as { reason: string }).reason).toContain("M1.1");
      expect(peek(root, "tester")).toBeNull(); // 没投出去

      // ── 3. dev 写产出（每条断言一行）再投 → 放行，tester 收到 ────
      p.file(
        "wf/dev-output-M1.md",
        `# dev 产出 M1\n\n- M1.1 已完成：src/hello.txt 已创建\n- M1.2 已完成：内容读起来是句人话\n`,
      );
      const good = { type: "review_request", milestone: "M1", body: "做完了", refs: ["wf/dev-output-M1.md"] };
      ...
      const retry = { type: "review_request", milestone: "M1", body: "补了判定行", refs: ["wf/dev-output-M1.md"] };
      expect(intercept.dev(retry)).toBeUndefined();
      await send.dev(retry, root);

      // tester 给报告（verdict_pass 需要 questions 覆盖 [human] 断言）
      const verdict = { type: "verdict_pass", milestone: "M1", questions: ["M1.2 内容读起来是句人话吗"] };
