...
- P8 S5 的编号规格曾自相矛盾：「解析时按出现顺序分配 id」与「删行不回收 id」互斥（按位置分配则删行必然回收）。真实 plan.md 是 P1 P2 P3 P5 P6 P7——P4 已删未回收，说明实际形态是「从文本读 id」。L7 按真实形态写了测试，规格文字已随 04-plan.md 拆解落在 `src/plan/grammar.ts` 的 S5 注释里（矛盾不存在了）。**待人确认后删行** —— [human] 归我 —— 前置：无
